...
const App: React.FC = () => {
  ...
  const handleBackClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    e.preventDefault();
    // 使用浏览器历史记录返回（因为现在是在同一窗口跳转，不是新窗口）
    if (window.history.length > 1) {
      window.history.back();
    } else {
      // 如果历史记录为空，尝试跳转到 navigation.html
      // 由于动画播放页面在 Vite 开发服务器上，需要构建正确的路径
      const currentOrigin = window.location.origin;
      // 尝试跳转到项目根目录的 navigation.html
      // 注意：这需要 navigation.html 也在可访问的位置
      window.location.href = currentOrigin.replace(':3000', '') + '/navigation.html';
    }
  };
